...
double monteCarloIntegration(const std::function<double(double)>& func, double lowerLimit, double upperLimit, int numSamples, int rank, int size) {
...
int main(int argc, char* argv[]) {
    ...
    MPI_Gather(&local_result, 1, MPI_DOUBLE, local_results.data(), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    if (rank == 0) {
        double result = 0.0;
        for (double r : local_results) {
            result += r;
        }
